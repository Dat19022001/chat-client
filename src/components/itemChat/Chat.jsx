...
import { setDeleteMess } from "../../redux/slices/test";
import { getMessageDB, sentMessageDB } from "../../services/chats";

import "./Chat.scss"

const Chat = ({ item, idUser }) => {
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      ...
      if (mqttClient && mqttClient.connected && value.trim() !== "") {
        ...
        const formattedMessage = `{value=${value.toString()}$id=${
          item.idFriend
        }}`;
        const formattedMessageT = { value: value, id: "user", isText: true };
        mqttClient.publish(`${item.idChat}`, formattedMessage);
        handle(formattedMessageT);
        setValue("");
        sentMess(value, idUser, item.idFriend);
      }
    }
  };
